...
export type MotionPresetLayer = 'primitive' | 'component' | 'layout' | 'overlay' | 'page';
...
function enterPreset(config: {
  ...
  return {
    antiUseCases: config.antiUseCases ?? [
...
const enterPresets = {
  ...
  'zoom-in-soft': enterPreset({
    durationToken: 'md',
    scaleToken: '96',
    transformFrom: combineTransforms(scale(cssVar(SCALE['96'])), translateY(cssVar(DISTANCE.xs))),
  ...
  'depth-in-soft': enterPreset({
    ...
    properties: 'opacity, transform, filter',
    tier: 'C',
    transformFrom: combineTransforms('translateZ(-24px)', scale(cssVar(SCALE['96']))),
...
const leavePresets = {
  ...
  'zoom-out-soft': leavePreset({
    durationToken: 'md',
    scaleToken: '96',
    transformTo: combineTransforms(scale(cssVar(SCALE['96'])), translateY(cssVar(DISTANCE.xs))),
  ...
  'depth-out-soft': leavePreset({
    ...
    properties: 'opacity, transform, filter',
    tier: 'C',
    transformTo: combineTransforms('translateZ(-24px)', scale(cssVar(SCALE['96']))),
