...
- Decide whether a new or changed public visual surface is lab-eligible and update the playground lab manifest accordingly.
- Keep Playwright retry policy local-vs-CI explicit and use retry-scoped traces instead of always-on trace capture.
- Add browser-level accessibility checks to curated Storybook or playground flows when they are load-bearing and stable enough to matter.
- Update copy serializers, preview schemas, and usage inspection data when visual props or variants change.
...
- Do not let lab coverage drift away from the governed visual surface list.
