...
- Runtime budget recalibration must stay evidence-backed: if GitHub Actions medians drift without a corresponding build-size or harness-contract regression, update the canonical catalog thresholds and ADR record instead of adding workflow-specific exceptions or parallel perf rules.
