...
for (const workspacePackage of workspacePackages) {
  if (!PACKAGE_CLASSIFICATION_MAP[workspacePackage.packageName]) {
    throw new Error(`Workspace package "${workspacePackage.packageName}" is missing from package classification.`);
  }
}

for (const packageEntry of PACKAGE_CLASSIFICATION) {
  if (!PACKAGE_LAYERS.includes(packageEntry.packageLayer)) {
    throw new Error(`Package "${packageEntry.packageName}" has invalid layer "${packageEntry.packageLayer}".`);
  }
  ...
  if (!workspacePackage) {
    throw new Error(`Classified package "${packageEntry.packageName}" does not exist in the workspace.`);
  }
  ...
  if (packageEntry.public && (!packageEntry.readmePath || !fileExists(packageEntry.readmePath))) {
    throw new Error(`Public package "${packageEntry.packageName}" must have a README at "${packageEntry.readmePath}".`);
  }
}

const publicPackages = PACKAGE_CLASSIFICATION.filter((entry) => entry.public).map((entry) => entry.packageName);

for (const packageName of publicPackages) {
  const surfaces = PUBLIC_SURFACE_MANIFEST.filter((entry) => entry.packageName === packageName);
  if (surfaces.length === 0) {
    throw new Error(`Public package "${packageName}" has no public surface entries in the manifest.`);
  }
}
...
for (const surface of PUBLIC_SURFACE_MANIFEST) {
  ...
  if (!packageMeta) {
    throw new Error(`Manifest entry "${surface.exportName}" references unknown package "${surface.packageName}".`);
  }
